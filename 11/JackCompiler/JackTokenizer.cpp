...
namespace helper
{
	...
	{
		if(currentToken.empty()) { return false; } // verifique se a string não está vazia
		...
		// number é um int, verifique o intervalo
		int temp = stoi(currentToken);
		assert(temp >= 0 && temp <= 32767);
		return true; // int está ao alcance, retorna true
	}
}
...
void JackTokenizer::advance(void)
{
	...
	while(currentToken_ == "//" || currentToken_ == "/*" || currentToken_ == "/**")
	{
		...
		else
		{
			...
			fin_ >> currentToken_; // obtenha o primeiro token da próxima linha
		}
	}
}
...
tokentype JackTokenizer::tokenType(void)
{
	...
	else if(!helper::isInt(currentToken_.substr(0,1))) { return IDENTIFIER; } // nome da variável não começa com um número
	else { cerr << "O token não é um tipo de token.\n"; }
}
...
string JackTokenizer::identifier(void) // identificador realizará uma atualização no currentToken_
{
	...
	else if(braket > -1 && braket < parens) // se o braket vem antes dos parens, então tome isso como prioridade
	{
		returnString += currentToken_.substr(0,braket);
		currentToken_ = currentToken_.substr(braket, currentToken_.length());
	}
	else if(parens > -1 && parens < braket) // mais se os parens é menor que o braket, então tome este
	{
	...
	else if(semi > -1) // semi é o último porque uma declaração sempre termina em ponto e vírgula
	{
...
string JackTokenizer::stringVal(void)
{
	...
	currentToken_ = currentToken_.substr(1,currentToken_.length()-1); // "Solte a primeira cotação"
	int endQuote = currentToken_.find('"');
	if(endQuote == -1) // Se a cotação final não for encontrada no currentToken_, encontre-a.
	{
		char c;
		fin_.get(c);
		while(c != '"')
	...
	else // se endQuote for encontrado, retorne a string antes da cota final e atualize o currentToken_ para ser o resto da string após a citação final
	{
