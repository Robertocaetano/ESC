...
public:
	...
	keyword keyWord(void); // Retorna a keyword que é o token atual. Deve ser chamado apenas quando tokenType for KEYWORD
	char symbol(void); // Retorna o caractere que é o token atual. Deve ser chamado apenas quando tokenType for SYMBOL
	string identifier(void); // Retorna o identificador que é o token atual. Deve ser chamado apenas quando tokenType for IDENTIFIER
	...
	string getCurrentToken(void); // Returna currentToken
	void updateCurrentToken(void); // Deixa fora o primeiro caractere de currentToken_
