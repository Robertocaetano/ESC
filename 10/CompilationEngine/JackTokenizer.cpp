...
namespace helper
{
	...
	{
		if(currentToken.empty()) { return false; } // verifique se a string nao está vazia
		...
		// numero é um int, verifique o intervalo
		int temp = stoi(currentToken);
		assert(temp >= 0 && temp <= 32767);
		return true; // int esta no alcance, return true
	}
}
...
JackTokenizer::JackTokenizer(string inputFile)
{
	...
	if(!fin_.is_open())
	{
		cerr << "JackTokenizer: falha ao abrir arquivo de entrada.\n";
	}
	currentToken_="";
}
...
void JackTokenizer::advance(void)
{
	...
	while(currentToken_ == "//" || currentToken_ == "/*" || currentToken_ == "/**")
	{
		...
		{
			getline(fin_,currentToken_); // pegue o resto da linha e exclua
			fin_ >> currentToken_; // obtenha o primeiro token da proxima linha
		}
	}
}
...
tokentype JackTokenizer::tokenType(void)
{
	...
	else if(!helper::isInt(currentToken_.substr(0,1))) { return IDENTIFIER; } // nome da variavel nao comeca com um numero
	else { cerr << "O token nao e um tipo de token.\n"; }
}
...
char JackTokenizer::symbol(void)
{
	...
	char returnChar = currentToken_[0];
	return returnChar;
}
