...
CompilationEngine::CompilationEngine(JackTokenizer * JT, string outputFile)
{
	...
	if(!fout_.is_open())
		cerr << "Falha ao abrir o arquivo de saída.\n";
	compileClass();
}
...
void CompilationEngine::compileLet(void)
{
	...
	fout_ << "<keyword> " << CEhelper::keyWord2String(JT_->keyWord()) << " </keyword>" << endl;
	JT_->advance();
	fout_ << "<identifier> " << JT_->identifier() << " </identifier>" << endl; // identificador realiza atualizacao em currentToken_
	if(JT_->getCurrentToken() == "")
		JT_->advance();
	if(JT_->symbol() == '[')
	{
...
void CompilationEngine::compileIf(void)
{
	...
	fout_ << "<keyword> " << CEhelper::keyWord2String(JT_->keyWord()) << " </keyword>" << endl;
	JT_->updateCurrentToken();
	JT_->updateCurrentToken(); // feito duas vezes desde que "if" é de 2 letras
	if(JT_->getCurrentToken() == "")
		JT_->advance();
	assert(JT_->symbol() == '(');
	fout_ << "<symbol> " << JT_->symbol() << " </symbol>" << endl;
